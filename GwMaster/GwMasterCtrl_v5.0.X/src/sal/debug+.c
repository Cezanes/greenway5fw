...
static const char * lvl_str[] = {"debg", "verb", "info", "warn", "erro"};

#if DBG_USE_COLORS == 1
   static const char * lvl_color[] = {"\033[0;37m", "\033[0;37m", "\033[1;32m", "\033[1;33m", "\033[1;31m"};
   static const char * def_color = "\033[0m";
#endif
   
   
#define uart_write_byte(x)  dbg.config.uart->write(dbg.config.uart->lcb, x)
...
static void dbg_uart_write(void * data, size_t count)
{   
   uint8_t * ptr = data;
   while (count > 0)
   {
...
static void dbg_uart_write_const(const void * data, size_t count)
{
...
void dbg_print(int lvl, const char * fmt, ...)
{
...
#if DBG_USE_COLORS == 1
   dbg_uart_write_const(lvl_color[lvl], 7);
#endif
   
   dbg_timestamp();
   
   dbg_uart_write_const(lvl_str[lvl], 4);
   dbg_uart_write(": ", 2);
   ...
   //dbg_uart_write(dbg.buff, size);
   
   dbg_println(lvl);
   
#if DBG_USE_COLORS == 1
   dbg_uart_write_const(def_color, 4);
#endif
}
...
void dbg_hex(int lvl, const char * text, const void * ptr, size_t size)
{
   ...
   while(*text)
      dbg_uart_write_const(text++, 1);
    
   for(size_t i = 0; i < size; i++)
   {
      //sprintf(dbg.buff, "%02X ", data[i]);
      //dbg_uart_write(dbg.buff, 3);
...
void dbg_print_hdr(int lvl, const char *fmt, ...)
{
  if(lvl < dbg.lvl || lvl >= COUNT(lvl_str))
   ...
   va_list ap; 
   va_start(ap, fmt); 
   //int size = vsnprintf(dbg.buff, sizeof(dbg.buff), fmt, ap);
   tfp_format(fmt, ap);
   va_end(ap);
