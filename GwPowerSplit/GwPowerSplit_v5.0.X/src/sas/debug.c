...
void dbg_init(const DebugConfig * config)
{
   ...
   dbg.config.uart->init(dbg.config.uart->lcb, dbg.config.clock_src, dbg.config.baud);
}
...
{  
   ...
   size_t len = sprintf(dbg.config.buff, "[%6u] ", timer_tick());
   dbg_uart_write(dbg.config.buff, len);
...
void dbg_print(int lvl, const char *fmt, ...)
{
   ...
   va_list ap; 
   va_start(ap, fmt); 
   int size = vsnprintf(dbg.config.buff, dbg.config.buff_size, fmt, ap);
   va_end(ap);
   
   dbg_uart_write(dbg.config.buff, size);
...
void dbg_hex(int lvl, const char *text, const void *ptr, size_t size)
{
   ...
   {
      sprintf(dbg.config.buff, "%02X ", data[i]);
      dbg_uart_write(dbg.config.buff, 3);
   }
